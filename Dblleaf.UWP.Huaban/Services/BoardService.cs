...
namespace Dblleaf.UWP.Huaban.Services
{
    ...
    {
        public BoardService(HbHttpHelper httpHelper)
            : base(httpHelper) { }
        ...
        public async Task<Board> add(String boardTitle, String category = "")
        {
            ...
            string json = await HttpHelper.Post(Constants.API_BOARDS, paramList.ToArray());
        ...
        public async Task<Board> edit(Board board)
        {
            ...
            string json = await HttpHelper.Post($"{Constants.API_BOARDS}{board.board_id}", paramList.ToArray());
            return board;
        }
        ...
        public async Task<string> follow(string boadID, bool follow)
        {
            ...
            return await HttpHelper.Post(uri);
        }
        ...
        public async Task<List<Pin>> GetPins(string boardID, long max = 0, int limit = 20)
        {
            ...
            string json = await HttpHelper.Get(uri);
            JObject obj = JObject.Parse(json);
